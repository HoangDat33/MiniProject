...
public class AcceptUserRegisterNotification extends BaseRequireAuthentication {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet AcceptUserRegisterNotification</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet AcceptUserRegisterNotification at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response, AccountLogin account) throws ServletException, IOException {
        ...
        if (account == null || account.getUser().getRole().getID() != 1) {
            ...
            rnDao.acceptRegister(idU);
            jsonResult.addProperty("valid", true);
            jsonResult.addProperty("message", "Cập nhật chấp thuận đăng kí thông báo thành công!");
        }
