...
<html lang="en">
    <head>
        ...
        <!-- site metas -->
        <title>The Card Shop - Danh sách đăng ký</title>
                                            ...
                                            <table class="table table-striped" style="margin-bottom: 15px">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th style="padding: 10px">Ngày gửi đơn</th>
                                                        <th style="padding: 10px">Chi tiết đơn</th>
                                                        <th style="padding: 10px">Trạng thái</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    <c:choose>
                                                        <c:when test="${not empty listMemRegis}">
                                                            <c:forEach var="member" items="${listMemRegis}">
                                                                <tr>
                                                                    <td style="padding: 10px">${member.getId()}</td>
                                                                    <td style="padding: 10px">${member.getUser().getLastName()} ${member.getUser().getFirstName()}</td>
                                                                    <td style="padding: 10px">${member.getUser().getEmail()}</td>
                                                                    <td style="padding: 10px"><fmt:formatDate value="${member.getCreatedAt()}" pattern="yyyy-MM-dd"/></td>
                                                                    <td style="padding: 10px">
                                                                        <form action="accessmember" method="post">
                                                                            <input type="hidden" name="rmId" value="${member.getId()}">
                                                                            <button class="btn btn-info" type="submit">Xem đơn</button>
                                                                        </form>
                                                                    </td>
                                                                    <td style="padding: 10px">
                                                                        <c:choose>
                                                                            <c:when test="${!member.isIsDelete()}">
                                                                                <span style="color: red;">Chưa được chấp nhận</span>
                                                                            </c:when>
                                                                            <c:otherwise>
                                                                                <span style="color: #4CAF50;">Đã được chấp nhận</span>
                                                                            </c:otherwise>
                                                                        </c:choose>
                                                                    </td>
                                                                </tr>
                                                        ...
                                                        <c:otherwise>
                                                            <tr>
                                                                <td colspan="5" style="padding: 10px; text-align: center;">Không có yêu cầu nào được ghi nhận</td>
                                                            </tr>
                                                        </c:otherwise>
                                                    </c:choose>
                                                </tbody>
